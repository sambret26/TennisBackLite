...
def get_players_infos(homologation_id):
    players_url = url_repository.get_by_label("Players").replace("HOMOLOGATION_ID", homologation_id)
    return moja_requests.send_get_request(players_url)

def get_teams_infos(homologation_id):
    teams_url = url_repository.get_by_label("Teams").replace("HOMOLOGATION_ID", homologation_id)
    return moja_requests.send_get_request(teams_url)

def get_call_ups(category_id):
    call_ups_url = url_repository.get_by_label("CallUps").replace("CATEGORY_ID", str(category_id))
...
def update_courts():
    ...
    if courts_information is not None and courts_information:
        ...
        court_repository.add_all([Court.from_fft(court) for court in courts_information['list']])

def update_categories():
    homologation_id = competition_repository.get_homologation_id()
    categories = moja_requests.send_get_request(url_repository.get_by_label("Category")
                                          .replace("HOMOLOGATION_ID", str(homologation_id)))
    if categories is not None and categories:
        grid_repository.delete_all()
        category_repository.delete_all()
        category_repository.add_all([Category.from_fft(category) for category in categories])

def update_grids():
    for category in category_repository.get_all():
        url = url_repository.get_by_label("CategoryData").replace("CATEGORY_ID", str(category.id))
        ...
        grid_repository.add_all(grids_to_add)

def update_rankings():
    rankings = moja_requests.send_get_request(url_repository.get_by_label("Rankings"))
    if rankings is not None and rankings:
        player_repository.delete_all()
        ranking_repository.delete_all()
        ranking_repository.add_all([Ranking.from_fft(ranking) for ranking in rankings])

def update_matches_by_grid(ctx, grid):
    if grid.grid_type == "POU":
        return None
    matches = []
    grid_url = url_repository.get_by_label("GridData").replace("GRID_ID", str(grid.table_id))
    matches_from_moja = moja_requests.send_get_request(grid_url)
    if matches_from_moja is None:
        return None
    sorted_matches = sorted(matches_from_moja, key=lambda x: get_match_tuple(x["numeroMatch"]))
    for match in sorted_matches:
        new_match = create_match(ctx, match, grid)
...
def get_match_tuple(numero_match):
    value_1 = int(numero_match.split('T')[1].split('M')[0])
    value_2 = int(numero_match.split('M')[1])
    value_3 = int(numero_match.split('Q')[1].split('T')[0])
    return value_1, value_2, value_3

def create_match(ctx, match, grid):
    new_match = Match.from_fft(match)
    new_match.category_id = grid.category_id
    new_match.grid_id = grid.id
    new_match.label = grid.category.code + str(ctx.match_index).zfill(2)
...
def set_winner(new_match, match):
    ...
    if new_match.double:
        if match["insIdWin"] == match['insId1']:
            new_match.team_winner_id = new_match.team1_id
        else:
            new_match.team_winner_id = new_match.team2_id
    else:
        if match["insIdWin"] == match['insId1']:
...
def get_court_information():
    ...
    return moja_requests.send_post_request_with_headers(url_repository.get_by_label("Courts"), data)
...
class UpdateMatchesContext:
    def __init__(self):
        ...
        self.updated = 0
        self.deleted = 0
        self.grids = grid_repository.get_all()
        self.matches_by_id_map = match_repository.get_by_id_map()
        self.new_matches = []
        self.temp_matches_map = {}
        self.next_grid_map = grid_repository.get_next_grids_map()
        self.grid_code_by_id_map = grid_repository.get_code_by_id_map()
