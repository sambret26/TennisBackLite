...
def update_competitions():
    competitions = moja_requests.send_get_request(url_repository.get_by_label("Competition"))
    if competitions is None:
        return None
    competitions_in_db = competition_repository.get_all()
    ...
    for competition_fft in competitions:
        ...
        if competition_in_db:
            if competition.are_different(competition_in_db):
                competition_repository.update(competition_in_db.id, competition)
            competitions_id_to_delete.remove(competition.id)
        else:
            competitions_to_add.append(competition)
    competition_repository.add_all(competitions_to_add)
    competition_repository.delete_all(competitions_id_to_delete)
    return 200
