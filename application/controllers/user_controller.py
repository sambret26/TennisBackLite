...
def update_role(user_id):
    ...
    if new_role < user.profile or user.super_admin == 1:
        if user.profile == 2:
            user = user_repository.update_profile(user_id, new_role, 1)
        else :
            user = user_repository.update_profile(user_id, new_role, user.super_admin)
        token = jwt.encode(user.to_dict(), SECRET_KEY, algorithm='HS256')
        return jsonify({'token': token}), 200
...
@user_bp.route('/<int:user_id>/access', methods=['POST'])
def ask_access(user_id):
    data = request.json
    role = str(data['role'])
    user = user_repository.get_by_id(user_id)
    profile = profile_repository.get_by_value(role)
...
@user_bp.route('/<int:user_id>/changePassword', methods=['PUT'])
