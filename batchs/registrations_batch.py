...
def handle_teams(ctx):
    ...
    if teams_ids_in_db:
        team_repository.delete_all_by_ids(teams_ids_in_db)
...
def add_categories_to_player(ctx, player, player_from_moja):
    for category in player_from_moja['epreuves']:
        ...
        map_key = "{}_{}".format(player.id, category['eprId'])
        if "insId" in category:
            ctx.player_categories_inscription_id_map[map_key] = category['insId']
...
def handle_new_categories(ctx, player, player_in_db):
    for category in player.categories:
        if category not in player_in_db.categories:
            map_key = "{}_{}".format(player.id, category.id)
            inscription_id = ctx.player_categories_inscription_id_map.get(map_key)
            ctx.players_categories_to_save.append(PlayerCategories(player.id, category.id, inscription_id))
...
def handle_old_categories(ctx, player, player_in_db):
    for category in player_in_db.categories:
        if category not in player_in_db.categories:
            map_key = "{}_{}".format(player.id, category.id)
            inscription_id = ctx.player_categories_inscription_id_map.get(map_key)
            ctx.players_categories_to_delete.append(PlayerCategories(player.id, category.id, inscription_id))
...
class RegistrationBatchContext:
    def __init__(self, send_notifications):
        ...
        self.categories_by_id_map = category_repository.get_by_id_map()
        self.players_by_id_map = player_repository.get_all_by_id_map()
        self.ranking_by_id_map = ranking_repository.get_by_id_map()
        self.messages = []
        ...
        self.player_categories_inscription_id_map = {}
